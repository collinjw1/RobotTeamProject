...
Authors: David Fisher and Isaiah Jolly.
...
def main():
    ...
    forward_button['command'] = lambda: send_drive(mqtt_client, left_speed_entry)
    root.bind('<Up>', lambda event: send_drive(mqtt_client, left_speed_entry))

    left_button = ttk.Button(main_frame, text="Left")
    left_button.grid(row=3, column=0)
    # left_button and '<Left>' key
    left_button['command'] = lambda: send_left(mqtt_client, right_speed_entry)
    root.bind('<Left>', lambda event: send_left(mqtt_client, right_speed_entry))
    ...
    root.bind('<Right>', lambda event: send_right(mqtt_client, left_speed_entry))
...
def send_drive(client, speed):
    ...
    client.send_message('drive', [speed])


def send_left(client, right_speed):
    print('Turning left')
    client.send_message('left_turn', [right_speed])


def send_right(client, left_speed):
    print('Turning right')
    client.send_message('right_turn', [left_speed])
...
def send_back(client, speed):
    print('Driving backwards')
    client.send_message('drive_back', [speed])
